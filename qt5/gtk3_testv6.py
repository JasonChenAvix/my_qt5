...
class FollowWindow(QWidget):
    def __init__(self):
        ...
        self.pidx = PIDController(4 , 0.004, 0.85,setpoint=0, sample_time=0.1 ) # for 6 frame
        self.pidy = PIDController(1.2 , 0.001, 0.006, setpoint=0 , sample_time=0.1 )
    ...
    def gimbal_callback(self, msg):
        ...
        if self.set_center:
            ...
            control_msg.pan_velocity = ctl_pan
            control_msg.tilt_velocity = ctl_tilt
            self.control_publisher.publish(control_msg)
    ...
    def calculate_control_command(self, yaw_angle,pitch_angle,max_yaw_angle,max_pitch_angle):
        ...
        if point_cx_dif>1 or point_cy_dif>1:
            self.get_logger().info(f'point_cx_dif {point_cx_dif} , point_cy_dif {point_cy_dif}')
        ...
        self.get_logger().info(f'speedxraw {speedx} , speedyraw {speedy}')
        
        # cap the speed to required format in KTG
        speed_x =min(max(speedx * 4000,-8000),8000) 
        speed_y =min(max(speedy * 4000,-8000),8000) 
        self.get_logger().info(f'speed_x {speed_x}, speed_y {speed_y}')
        
        # 600 is threshold
        if abs(speed_x)<600 and abs(speed_x)>350:
            if speed_x>0:
                speed_x=600
            else:
                speed_x=-600
        
        if abs(speed_y)<600 and abs(speed_y)>350:
            if speed_y>0:
                speed_y=600
            else:
                speed_y=-600
